...
**Day 1 (Jun 24)** ✅
- [x] Create `app/skills/` package; move `tools.py` → `app/skills/decodability.py` (`tools.py` kept as re-export shim; `agent.py` import rewired)
- [x] Define `GraphemeHit` and `WordDecomposition` dataclasses
...
- [ ] `phonics_db`: grapheme inventory + BKT params (`p_init,p_transit,p_slip,p_guess`)

**Day 4 (Jun 27)**
- [ ] `app/store/learner_store.py`: interface + JSON impl (SQLite optional)
- [ ] Curriculum planner: deterministic ZPD target + spaced-review picks → `Objective`
- [ ] Unit tests: mastery rises on correct / falls on errors; planner picks lowest unmastered

**Done when:** evidence → sane mastery update; planner returns next target; profile round-trips through store.
...
- [ ] Miscue classification: substitution/omission/insertion/self_correction/hesitation
