# Demo narration — ROUGH FIRST DRAFT (edit Jul 4–5)

> **Record-ready draft (TODOs closed + timed Jul 5).** Spoken beats only, timed to ≈5:00.
> Words to *say*, matched to the VERIFIED runbook click-path (`docs/demo-runbook.md`), not the
> stale Shot-1 choreography in `video-skeleton.md`. `[SCREENSHOT:…]` / `[CITE:…]` placeholders
> are deliberate (media capture is a separate task).
> Tone target: the "knowledgeable neighbor" — warm, plain, honest, never salesy (`voice-lexicon.md`).
> Shot order + timing follow `video-skeleton.md`; UI choreography follows the runbook.
> NUMBERS LOCKED: **full offline test suite green** (count-free — say "the whole offline suite passes," not a specific number); adapt chain **wh→ck→qu** via **"One miscue"**; evidence **+0.06 acc / +0.04 mean mastery / +4.9 WCPM**.
> DO NOT say the never-punish confidence repair "fires live" — it's a no-op on the Gemini Live path.

---

## Shot 0 — Cold open / problem (0:00–0:30, ~30s)
**On screen:** title card "StoryForge · by Phono" + problem line — **`artifacts/media/title-card.png`** (full-screen, whole shot).

**Say (timed Jul 5 — ~69 words ≈ 28s @150wpm):**
- "If your child struggles to read, you get one piece of advice: decodable books, at their level."
- "Two problems. The right book — your kid's phonics level, age, and interests — basically doesn't exist on a shelf."
- "And nothing is watching *what* they misread to decide what to practice next. There's no loop."
- "Phono StoryForge closes that loop — it generates the book, listens to the read, and moves the target itself." [CITE: README "The Problem" README.md:7–9]

---

## Shot 1 — LIVE web demo: the loop adapting (0:30–2:15, ~1:45) ★ centerpiece
**On screen:** browser at `http://127.0.0.1:8000/?loop&demo` (`demo` slows the adapt beat 1.5× for camera), seeded **`ada`** returning-reader profile.
Reading face → score "One miscue" → "For grown-ups" → Insight face → second read.
**(post-rebrand UI: mastery is a node PATH, heatmap lives in the Insight face, loop rail visible because of `?loop`)**

**Say (trimmed + timed Jul 5 — ~163 words ≈ 65s @150wpm, leaving ~40s for clicks + the adapt beat to land in silence) — as you land + start the read:**
- "This is the actual product running live — the same code the experiment runs."
- "Ada's story tonight is built from the sounds she's ready for — the targets are softly underlined." [SCREENSHOT: Reading face, gold-underlined graphemes]
- "She reads the page — I'll score a typical read from the keyboard so it's repeatable on camera." (Click: expand "⌨ Type or use a preset" → **"One miscue"** → "How did it go? ▸")

**After scoring, before toggling:**
- "First, the kid view — a clean win, no scores, no red ink." (Session Arc: Read together → How it went)

**Tap "For grown-ups" (key `i`), Insight face appears — the adapt beat plays ON ENTRY (it's deferred until Insight opens; you can't miss it — let it land in silence, ~2s):**
- "Now the grown-up view."
- "**Why card:** it didn't pick this story at random — here's why it chose this sound and this book." [SCREENSHOT: Why card] [CITE: planner rationale, README.md:15–24]
- "**Running record:** the miscue is attributed to the exact sound she missed — that red cell is the grapheme she stumbled on." [SCREENSHOT: heatmap, 1 red cell]
- "**Mastery path:** the sound she just nailed pops, and the glow travels to the next target." (the caption resolves on its own — don't read it aloud)
- "One read: miscue attributed, mastery updated, tomorrow's target moved — **wh → ck**."

**Second read, to prove it's a loop not a one-shot:**
- "Once more on the new book." (Back to Reading `r` → "One miscue" again → back to Insight)
- "The target moves again — **ck → qu**. The loop adapted twice, live."
- "That rail along the top — Plan, Generate, Verify, Read, Assess, Adapt — is the ADK loop you just watched fire." (visible because we opened with `?loop&demo`; horizontal rail directly under the top bar)

**RUNBOOK NOTE:** if the adapt animation doesn't visibly move → cut to 🏷 Shelf/Journey timeline (sound-by-sound shift across sessions) as the alternate adapt proof.

---

## Shot 2 — CUT (was: browser-mic voice)
**Decided Jul 5: voice shot removed.** Typed presets are the recorded path; the honesty line in
Shot 1 ("scoring from the keyboard so it's repeatable on camera") covers it. Do NOT click
"🎤 Read aloud" during recording — with unverified Gemini Live creds it hangs on "transcribing…"
and wedges the WebSocket (runbook Shot 2). Shot numbering 3–5 unchanged.

---

## Shot 3 — Offline evidence chart: rigor (2:15–3:45, ~1:30)
**On screen:** full-screen `eval/experiments/results/adaptive_vs_static.png`. (optional: regen command in terminal)

**Say (rough) — the result:**
- "Does adapting actually help, or does it just look clever? We tested it."
- "Same paired, seeded simulated learners. Same fixed benchmark probe every session. Adaptive — picks each session's target from the child's estimate — versus a fixed scope-and-sequence that ignores the evidence."
- "n=30, 40 sessions: **probe accuracy +0.06, true mean mastery +0.04, +4.9 words-correct-per-minute.**" [SCREENSHOT: chart headline]

**Say (rough) — the rigor sub-beat (DON'T skip):**
- "And we didn't let it grade its own homework. The simulated learner was built deliberately *unlike* what the tutor assumes — a logistic/IRT reader with per-sound difficulty, no prerequisite gate, and it *forgets*."
- "De-circularizing roughly halved the gaps — but they stayed positive across all 12 cells of a forgetting-by-difficulty sweep." [CITE: stage-d-independent-learner.md:29–55]

**Say (rough) — honesty beat (KEEP — reads as credible):**
- "One place it's a wash: the count of sounds pushed past a hard 0.95 bar. We log that in the CSV. We don't hide it." [CITE: README.md:79]

**RUNBOOK ⚠:** confirm the committed PNG is the de-circularized re-run before recording.

---

## Shot 4 — Illustrated book: polish (3:45–4:35, ~50s)
**On screen:** `results/sample_book/page_01.png … page_06.png` ("Sam the Fox", 6 pages). Prefer committed PNGs or a pre-captured Doc screen-recording over live `gws`. [SCREENSHOT: book pages]

**Say (rough):**
- "Every page is guaranteed made-from-sounds-they-know. An LLM proposes the words; deterministic Python verifies."
- "Three times, same discipline: a decodability check that won't let an undecodable word ship, a palette verifier that proves every illustration stays on-brand, and a Doc assembled deterministically." [CITE: README content engine README.md:46–71]
- "LLM proposes, code verifies — three guardrails that halt instead of quietly shipping something wrong."
- "And as of last week, this is folded into the live loop — not a separate path." [CITE: commit e7d815b]

**RUNBOOK ⚠:** if a live Doc stalls → fall back to committed page PNGs / pre-captured recording.

---

## Shot 5 — Close (4:35–5:00, ~25s)
**On screen:** architecture loop diagram / recap card; "**full offline suite green**" line. [SCREENSHOT: loop diagram] [CITE: README.md:15–24]

**Say (rough):**
- "One deterministic loop — plan, generate, listen, attribute, update — wired into a real product."
- "Proven by a de-circularized study, and now generating illustrated books inside that same loop."
- "**The whole offline unit suite passes green.** Built on ADK, for Agents for Good."
- Closing line (locked): "A five-minute win beats a fifteen-minute fight — at scale."

---

## Timing check
| Shot | Beat | Dur | Running |
|---|---|---|---|
| 0 | Problem | 0:30 | 0:30 |
| 1 | Live demo (centerpiece) | 1:45 | 2:15 |
| 2 | CUT (was voice) | — | — |
| 3 | Evidence + rigor | 1:30 | 3:45 |
| 4 | Illustrated book | 0:50 | 4:35 |
| 5 | Close | 0:25 | 5:00 |

≈ **5:00** (Shot 2 / voice is CUT — no trim trade-off needed).

**Timing verified Jul 5 (word-count at 150 wpm):** Shot 0 ≈ 28s/30 · Shot 1 ≈ 65s talk + ~40s
clicks/beats in 105 (trimmed from 247 → ~163 words) · Shot 3 ≈ 52s/90 · Shot 4 ≈ 29s/50 ·
Shot 5 ≈ 23s/25. Shots 3–4 have deliberate slack for the chart linger and page turns. Still do
one stopwatched read-aloud before recording — word-count timing doesn't capture breath or clicks.
